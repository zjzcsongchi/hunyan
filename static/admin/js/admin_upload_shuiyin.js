	    ...
	    uploadSuccess: function(event, file, serverData){
	        ...
	        if(data.error == 0){
	        	...
	        	if($(this).attr('data-type') == 'video'){
	        		...
	        		$("#"+file.id).html(html);
	        	}else if($(this).attr('data-type') == 'rich_text'){
	        		html += "<a class='close del-pic' href='javascript:;'></a>";
	        		html += "<img src='"+static_url+data.sy_url+"' style='width: 100%; height: 100%' />";
	        		html += "<input type='hidden' name='"+name+"' value='"+data.url+"'/>";
	        		html += "<input type='hidden' name='"+'sy_'+name+"' value='"+data.sy_url+"'/>";
	        		html += "<input type='hidden' name='"+'ys_'+name+"' value='"+data.ys_url+"'/>";
	        		$("#"+file.id).html(html);
	        	}
	        		...
	        		html += "<input type='hidden' name='"+'sy_'+name+"' value='"+data.sy_url+"'/>";
	        		html += "<input type='hidden' name='"+'ys_'+name+"' value='"+data.ys_url+"'/>";
	        		html += "<input type='hidden' name='"+'yt_'+name+"' value='"+data.yt_sy+"'/>";
	        		$("#"+file.id).html(html);
	        	}
	        }else{
		...
		swfupload: function (object){
			$.each(object, function(key, value) {
		    	...
		    	if(value.type == 'rich_text'){
		    		$(value.obj).swfupload({
		    			upload_url: uploadUrl+"/file/upload_shuiyin/1",
		    			file_size_limit : max(),
		    		...
		    		$(value.obj).swfupload({
		    			upload_url: uploadUrl+"/file/upload_shuiyin",
		    			file_size_limit : max(),
		    			file_types : "*.jpg;*.png;*.gif;*.mp4;*.jpeg",
		    			file_types_description : "All Files",
		    			file_upload_limit : "0",
