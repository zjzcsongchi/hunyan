	...
	module.exports={
			...
			save_order:function(){
				$('#save_2').click(function(){
					...
					var order_man = $("#order_man").val();
					if(order_man ==''){
						module.exports.msg('客户名称不能为空');
						return false;
					}
					var man_phone = $("#man_phone").val();
					if(man_phone ==''){
						module.exports.msg('手机号不能为空');
						return false;
					}
					var order_info = $("#order_info").val();
					var receptionist = $("#receptionist").val();
					if(receptionist == ''){
						module.exports.msg('接待人不能为空');
						return false;
					}
					var venue_ids = [];
					$('input:checked').each(function(k,v){
						venue_ids.push($(v).val())
					})
					if(venue_ids == ''){
						module.exports.msg('请选择场馆');
						return false;
					}
					var order_type = $('#order_type').val();
					if(order_type == ''){
						module.exports.msg('请选择婚宴类型');
						return false;
					}
					var g_time = $('#g_time').val();
					if(g_time ==''){
						module.exports.msg('请选择公历时间');
						return false;
					}
					var n_time = $('#n_time').val();
					if(n_time ==''){
						$('#n_time').val('计算中....');
						module.exports.msg('请等待系统计算农历时间');
						return false;
					}
					var week = $('#week').val();
					var start_time = $("#start_time").val();
					var bargain_money = parseFloat($('#bargain_money').val());
					if(bargain_money < 0){
						module.exports.msg('定金不能小于0');
						return false;
					}
                    var data = {'order_man':order_man,'man_phone':man_phone,'order_info':order_info,'receptionist':receptionist,'place_id':venue_ids,'order_type':order_type,'g_time':g_time,'n_time':n_time,'week':week,'start_time':start_time,'bargain_money':bargain_money};
					$.post('/drinkorder/addorderinfo',data,function(data){
						if(data.code == 1){
							$('#order_id').val(data.id); 
                            $('#order_num').val(data.order_num);
                            module.exports.msg('添加订单信息成功');
                          //隐藏第一个div,显示第二个div
			...
			edit_order:function(){
				$('#edit_order').click(function(){
                    ...
                    var data = {'id':id,'order_man':order_man,'man_phone':man_phone,'order_info':order_info,'receptionist':receptionist,'place_id':venue_ids,'order_type':order_type,'g_time':g_time,'n_time':n_time,'week':week,'start_time':start_time,'bargain_money':bargain_money};
					$.post('/drinkorder/editorderinfo',data,function(data){
						if(data.code == 1){
                            module.exports.msg('修改成功');
                            //修改预约金后自动更新合计的信息
                            var total_price = 0;
...
//					});
			...
			sum:function(){
				...
				var bargain_money = $("#bargain_money").val(); 
				var free = $("#free").val(); 
				
				need_price = total_price-bargain_money-free;
				need_price = parseFloat(need_price).toFixed(2);
				if(need_price<0){
					need_price=0;
				}
				$("#need").text(need_price);
				$("input[name='need_pay']").val(need_price);
			...
			save:function(){
				$(document).on('click','#save_2', function(){
					var ser_data = $('#form').serialize(); 
					$.post('/drinkorder/edit_goods/'+order_id,ser_data, function(data){
						module.exports.msg(data.data);
					})
...
//					var b = parseFloat($('#bargain_money').val());
//					var b = b ? b:0;
//					$('#need').html( parseFloat($('#total').html()) - parseFloat($(this).val()) - b );
				});
				
				$('#bargain_money').blur(function(){
					module.exports.sum();
//					if(!$(this).val()){
//						$(this).val(0);
//					}
//					var c = parseFloat($('#free').val());
//					var c = c ? c:0;
//					$("#need").text(parseFloat($('#total').html()) - parseFloat($(this).val()) - c);
				})
			},
