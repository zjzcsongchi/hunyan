	...
	module.exports={
			...
			del:function(){
				$('.del').click(function(){
					...
					var d = dialog({
						...
						ok:function(){
							var obj = this;
							$.post('/invitelement/del', {template_id:template_id,user_id:user_id}, function(data){
								if(data.status != 0){
				...
				$('.express_btn').click(function(){
					...
					var d = dialog({
							...
							ok:function(){
								...
								$.post('/invitelement/page/'+user_id+'/'+template_id, post_data, function(data){
									if(data.status != 0){
										d1.content(data.msg);
									}else{
										var d = dialog({
				...
				$('.element').click(function(){
					...
					var d = dialog({
							...
							ok:function(){
								...
								$.post('/invitelement/element/'+page_id, post_data, function(data){
									if(data.status != 0){
										d1.content(data.msg);
									}else{
										var d = dialog({
			...
			del_element:function(){
				$('.del').click(function(){
					...
					var d = dialog({
						...
						ok:function(){
							var obj = this;
							$.post('/invitelement/element_del', {id:id}, function(data){
								if(data.status != 0){
									obj.content = data.msg;
								}else{
									window.location.reload();
			...
			message_del:function(){
				$('.del').click(function(){
					...
					var d = dialog({
						...
						ok:function(){
							var obj = this;
							$.post('/invitelement/message_del', {id:id}, function(data){
								if(data.status != 0){
									obj.content = data.msg;
								}else{
									window.location.reload();
			...
			message_edit:function(){
				$('#submit').click(function(e){
					...
					var data = $('form').serialize();
					$.post('/invitelement/message_edit?id='+id, data, function(data){
						var content = data.msg;
						var d = dialog({
							title:"提示",
							content:content,
							ok:function(){
								if(data.status == 0){
									window.location.href="/invitelement/message/"+host_id;
									return false;
								}
							},
							okValue:"确定"
