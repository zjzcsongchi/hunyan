...
define(function(require, exports, module){
	...
	module.exports = {
		...
		add:function(edit_obj){
			var obj = this
			var content = $(add_voice);

			layer.create_layer("添加音乐热点", content)
			var object = [
          	...
          	$(".save_layer").click(function(){
          		...
          		if(is_dynamic){
                    ...
                    var source_url = $("input[name=music]").val()
          		var hotspot_value = {
          			hotspot_type:"voice",
		...
		edit:function(edit_obj, hotspot){
               ...
               //修改音乐
               var voice_obj = $("<li class='pic pro_gre'></li>")
               voice_obj.css({"margin-right":"20px", "clear":"none"})
               var voice_a = $("<a class='close del-pic'></a>")
               voice_obj.append(voice_a)

               var voice_img = $("<audio>")
               voice_img.attr("src", edit_obj.get_img_url(hotspot.source_url))
               voice_img.attr("controls", "controls")
               voice_img.css({"width":"100%", "margin-top":"128px"})
               voice_obj.append(voice_img)

               var voice_input = $("<input type='hidden' name='music'>")
               voice_input.attr("value", hotspot.source_url)
               voice_obj.append(voice_input)

               content.find("#uploader_music").prepend(voice_obj)


               layer.create_layer("修改音乐热点", content)
               //初始化添加图片按钮
               var object = [
                        {"obj":"#uploader_ico", "btn":"#btn_ico"},
                        {"obj": "#uploader_music", "btn": "#btn_music"}
