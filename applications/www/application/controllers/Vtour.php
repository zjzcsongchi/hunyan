...
class Vtour extends MY_Controller{
    ...
    public function index($id = 0){
        ...
        if($is_mobile){
            redirect(C('domain.mobile.url').'/vtour/scan/'.$id);
        }
        $id = $id ? $id :C('public.vtour.default_id');
        $data['id'] = intval($id);
...
//         }
    ...
    public function scan($id, $source = ''){
        $data = $this->data;
        $data['id'] = intval($id);
        $data['source'] = $source;
        //判断是否手机端
        ...
        if(IS_POST){
            ...
            if($this->Mvtour_comment->create($inser_data)){
                $this->return_success(array(
                    ...
                    'headurl' => !empty($this->data['user_info']['head_img']) ? get_img_url($this->data['user_info']['head_img']) : '%SWFPATH%/skin/head.png'
                ));
            }else{
