...
class Album extends MY_Controller{
    ...
    public function __construct(){
        ...
        //查寻用户优惠卷的数量
        $this->data['coupon_num'] = $this->Muser_coupon->count(['user_id' => $this->data['user_info']['id'], 'status' => C('coupon.status.no_use.id')]);
    ...
    public function index(){
        ...
        */
        $field = 'id, name';
        //该相册上一个的关联相册
        $where = array('dinner_id' => $album['dinner_id'], 'is_del' => 0, 'create_time <' => $album['create_time']);
        $order_by = array('create_time' => 'desc');
        $prev_album = $this->Mdinner_album->get_lists($field, $where, $order_by, 1);
        $data['prev_album'] = isset($prev_album[0]) ? $prev_album[0] : '';
        //该相册下一个的关联相册
        $where = array('dinner_id' => $album['dinner_id'], 'is_del' => 0, 'create_time >' => $album['create_time']);
        $order_by = array('create_time' => 'asc');
        $next_album = $this->Mdinner_album->get_lists($field, $where, $order_by, 1);
        $data['next_album'] = isset($next_album[0]) ? $next_album[0] : '';
    ...
    public function checkout(){
        ...
        if($this->input->is_ajax_request()){
            ...
            $order_date = array(
                ...
                'order_type' => isset($post_data['type']) ? $post_data['type'] : C('order_type')['image']['id'],
