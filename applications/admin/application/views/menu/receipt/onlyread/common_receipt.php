...
<?php case C('milan_staff_type.emcee.id'):?>
    	...
    	<p style="text-indent:20px;">
        	...
        	于 <mark><?php echo $menu['year']?>年<?php echo $menu['month']?>月<?php echo $menu['day']?>日</mark>，
        	 ...
        	 负责宴会庆典的彩排、主持仪式服务工作，望按时保质保量圆满完成此项工作。
    	 </p>	
...
<p class="text-center lead">
        ...
        <p style="text-indent:20px;">
        	...
        	共计   <mark><?php echo isset($receipt['other']['staff_count']) ? $receipt['other']['staff_count'] : '';?> </mark>人，
    	          于  <mark><?php echo isset($receipt['start_time'])&&$receipt['start_time'] ? $receipt['start_time'] : '' ?> </mark>
        	至 <mark><?php echo isset($receipt['end_time'])&&$receipt['end_time'] ? $receipt['end_time'] : '' ?> </mark>，
        	
        	 <?php $this->load->view('milan_schedule/receipt/venue_info_by_type')?>
        	负责<mark><?php echo isset($receipt['duty']) ? $receipt['duty'] : '';?></mark>
        	摄像及制作工作，望按时保质保量圆满完成此项工作。        
         </p>
...
<p class="text-center lead">
    	...
    	<p style="text-indent:20px;">
        	...
        	于 <mark><?php echo $menu['year']?>年<?php echo $menu['month']?>月<?php echo $menu['day']?>日</mark>，
        	<?php $this->load->view('milan_schedule/receipt/venue_info_by_type')?>
        	 负责<mark><?php echo isset($receipt['duty']) ? $receipt['duty'] : '';?></mark>声光电技术工作，望按时保质保量圆满完成此项工作。 
    	 </p>
...
<p class="text-center lead">
			...
			<p style="text-indent:20px;">
			    ...
			    于 <mark><?php echo $menu['year']?>年<?php echo $menu['month']?>月<?php echo $menu['day']?>日</mark>，
			    <?php $this->load->view('milan_schedule/receipt/venue_info_by_type')?>
			     负责宴会彩妆造型的服务工作，望按时保质保量圆满完成此项工作。</p>
			<div class="form-group">
			<ul>
			    <li>服务时间：化妆师将于  <mark><?php echo isset($receipt['other']['service_time1'])&&$receipt['other']['service_time1'] ? str_replace('T', ' ', $receipt['other']['service_time1']) : '' ?>  </mark> 到百年婚宴  <mark><?php echo isset($receipt['other']['service_room'])&&$receipt['other']['service_room'] ? $receipt['other']['service_room'] : '' ?> </mark> 化妆间，化妆流程至最后一个妆面至婚宴结束。</li>
			    <li>服务时间：化妆师将于  <mark><?php echo isset($receipt['other']['service_time2'])&&$receipt['other']['service_time2'] ? str_replace('T', ' ', $receipt['other']['service_time2']) : '' ?> </mark>到新娘家化妆，化妆流程至最后一个妆面至婚宴结束。</li>
			    <li>服务内容：
...
<p class="text-center lead">
    	...
    	<p style="text-indent:20px;">
        	...
        	<?php $this->load->view('milan_schedule/receipt/venue_info_by_type')?>
        	 负责<mark><?php echo isset($receipt['duty']) ? $receipt['duty'] : '';?></mark>跟拍及制作工作，望按时保质保量圆满完成此项工作。        
    	 </p>
...
<p class="text-center lead">
    	...
    	<p style="text-indent:20px;">
        	...
        	于 <mark><?php echo $menu['year']?>年<?php echo $menu['month']?>月<?php echo $menu['day']?>日</mark>，
        	 <?php $this->load->view('milan_schedule/receipt/venue_info_by_type')?>
        	 负责<mark><?php echo isset($receipt['duty']) ? $receipt['duty'] : '';?></mark> 场景布置和现场执行工作，望按时保质保量圆满完成此项工作
    	 </p>
...
<?php endswitch;?>
        	...
        	<div class="cont" style="margin-top:10px; font-size: small;">
                <div class="cont50">米兰套餐：<o style="boder:1px solid #333"><?php echo $menu['menus']?></o></div>
                <div class="cont50">婚宴主题：<o><?php echo $menu['theme']?></o></div>
                
                <div class="cont50">宴会负责人：<o><?php echo $menu['responsible_person']?></o></div>
                <div class="cont50">套系价格：<o> <?php echo $menu['menus']?> </o></div>
            </div>
            
            <div class="cont" style="margin-top:10px; font-size: small;">
                <div class="cont50">此次费用共计￥：<o><?php echo isset($receipt['money']) ? $receipt['money'] : '';?></o>元</div>
                <div class="cont50">大写人民币： <o><?php echo isset($receipt['chinese_money']) ? $receipt['chinese_money'] : '';?> </o></div>

            </div>

    	    <div class="cont" style="margin-top:10px; font-size: small;">
                <div class="cont50">经办人签字：<o><?php echo isset($receipt['operator']) ? $receipt['operator'] : ''; ?></o></div>
                <div class="cont50">执行人签字：<o><?php echo isset($receipt['status']) && $receipt['status']!=0 ? $staff_name : ''; ?></o></div>
                
                <div class="cont50" >开单日期：<o><?php echo isset($receipt['create_time']) ? $receipt['create_time'] : ''; ?></o></div>
                <div class="cont50">签字日期：<o><?php echo isset($receipt['confirm_time']) ? $receipt['confirm_time'] : ''; ?></o></div>
            </div>
<!--endprint-->
			<div class="col-sm-12 text-center" style="margin-top:10px;">
			    <a class="btn btn-primary examination no_print" data-receipt_id="<?php echo $receipt['id']?>" data-examination_suatus="<?php echo $receipt['examination_status']?>" data-examination_reson="<?php echo $receipt['examination_reson']?>" style="margin-bottom:5px">审核</a>
			    
                <a class="btn btn-primary print-preview no_print" style="margin-bottom:5px">打印</a>
            </div>
