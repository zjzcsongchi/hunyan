...
class MY_Controller extends CI_Controller {
    ...
    public function __construct() {
        ...
        if(!empty($_SERVER['HTTP_ORIGIN']) && in_array($_SERVER['HTTP_ORIGIN'], array_column(array_reverse($this->data['domain']),  'url'))){
        ...
        if (ENVIRONMENT === 'development') {
            $user_info = array('id' => 154, 'mobile_phone'=>'18302488851');
        ...
        {
            $login_page_arr = C('mobile_login'); //需要强制登录的页面
            ...
            if (array_key_exists($ctrl,$login_page_arr) && in_array($act, $login_page_arr[$ctrl]))
            {
                ...
                header('location:' . C('domain.mobile.url') . C('user_center.url.redirect_wechat_login'));
                exit;
            }
        }
        
        $config_news = C("news");
        $config_news_id = array_column($config_news, 'id');
        if($config_news_id){
            //获取一级分类
            $news = $this->Mnewsclass->get_lists('id, name', array('in'=>array('id'=>$config_news_id)));
            $this->data['first_news_class'] = array_column($news, 'name', 'id');
            //获取二级分类
            $second = $this->Mnewsclass->get_lists('id, name, parent_id', array('in'=>array('parent_id'=>$config_news_id), 'is_del'=>0));
            $second_tmp = array();
            foreach ($second as $k=>$v){
                $second_tmp[$v['parent_id']][$v['id']] = $v;
            }
            $this->data['second_news_class'] = $second_tmp;
        }
        $this->data['about'] = $this->Mabout_us->get_one('*', array());
        //获取手机底部菜单
        $wap_menus = $this->Mwap_urls->get_lists('*', ['is_del' => 0, 'is_show' => 1], ['sort' => 'desc']);
        if($wap_menus){
            $this->data['wap_menus'] = $this->myloop($wap_menus);
        }
    }
